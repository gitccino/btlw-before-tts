...
export function injectPauseMarkersFromWords(
  ...
  const chunkWords = words.filter(
    (w) =>
      w.start * 1000 >= chunk.startMs && w.end * 1000 <= chunk.endMs
  );

  if (chunkWords.length === 0) {
    return { text: chunk.englishText, count: 0, positions: [] };
  }
...
export function injectPauseMarkersFromPunctuation(chunk: Chunk): PauseResult {
  ...
  return { text, count, positions: [] }; // positions not available without word timestamps
}
